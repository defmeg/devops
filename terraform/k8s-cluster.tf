...
resource "yandex_storage_bucket" "backup-backet-def" {
  bucket = "backup-backet-def"
  force_destroy = true
...
# Compute instance group for srv

resource "yandex_compute_instance_group" "k8s-srv" {
  name               = "k8s-srv"
  service_account_id = yandex_iam_service_account.admin.id
  depends_on = [
    yandex_iam_service_account.admin,
...
output "instance_group_srv_public_ips" {
  description = "Public IP addresses for srv-nodes"
  value = yandex_compute_instance_group.k8s-srv.instances.*.network_interface.0.nat_ip_address
}

output "instance_group_srv_private_ips" {
  description = "Private IP addresses for srv-nodes"
  value = yandex_compute_instance_group.k8s-srv.instances.*.network_interface.0.ip_address
}
